########################################
Building and uploading cftime wheels
########################################
...
https://ci.appveyor.com/project/matthew-brett/cftime-wheels
...
* builds a cftime wheel, linking against these fresh builds;
...
* making a commit to the `cftime-wheels` repository (e.g. with `git
...
Which cftime commit does the repository build?
==================================================

The ``cftime-wheels`` repository will build the commit specified in the
...
be something like::
    ...
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t macosx cftime $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t manylinux1 cftime $VERSION
    wheel-uploader -r warehouse -u $CDN_URL -s -v -w ~/wheelhouse -t win cftime $VERSION
...
``cftime`` is the root name of the wheel(s) to download / upload, and
...
``cftime-0.4.0-`` from http://wheels.scipy.org to ``~/wheelhouse``,
